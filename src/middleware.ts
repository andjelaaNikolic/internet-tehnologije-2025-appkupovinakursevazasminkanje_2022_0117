...
export async function middleware(request: NextRequest) {
  ...
  // Dohvatanje tokena iz headera ili kolačića
  let token: string | undefined;
  const authHeader = request.headers.get("authorization");
  if (authHeader?.startsWith("Bearer ")) token = authHeader.substring(7);
  if (!token) token = request.cookies.get("auth")?.value;

  const isProtectedRoute =
    pathname.startsWith("/stranice/dodaj-kurs") || pathname.startsWith("/profil");
  const isLoginPage = pathname === "/login";

  // Redirekcija za stranice
  if (isProtectedRoute && !token) {
    const loginUrl = new URL("/login", request.url);
    loginUrl.searchParams.set("callbackUrl", pathname);
    return NextResponse.redirect(loginUrl);
  ...
  if (pathname.startsWith("/api")) {
    ...
    if (pathname.startsWith("/api/edukator") && uloga !== "EDUKATOR") {
      return addSecurityHeaders(
        NextResponse.json({ message: "Pristup dozvoljen samo edukatorima." }, { status: 403 })
      );
    }
